...
- **已知缺口**（设计稿有、原生菜单未提供）：原始共 10 项。本轮补齐 3 项——`关闭标签页`(⌘W) / `关闭其它标签页`(⇧⌘W)（`181ce35`，会话菜单，单标签置灰）、`选择选择内容`（`dcb8778`，编辑菜单，只读会话置灰；同时把已存在但未注册的`选择内容和剪贴板比较`一并注册）。**仍缺 5 项**：`剪切 / 复制 / 粘贴 / 删除 / 全选`（需文本编辑能力：TextEditTab 用 `egui::TextEdit` 可通过注入 egui 事件实现；MergeTab 无选区模型，需先补）与 `移动标签页到新窗口 / 合并所有窗口`（需多窗口架构，当前 GUI 未使用 egui viewport）。
