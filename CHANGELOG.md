...
- **已知缺口**（设计稿有、原生菜单未提供）：原始共 10 项。本轮补齐 3 项——`关闭标签页`(⌘W) / `关闭其它标签页`(⇧⌘W)（`181ce35`，会话菜单，单标签置灰）、`选择选择内容`（`dcb8778`，编辑菜单，只读会话置灰；同时把已存在但未注册的`选择内容和剪贴板比较`一并注册）。**仍缺 7 项**：`剪切 / 复制 / 粘贴 / 删除 / 全选`（5 项，需文本编辑能力：TextEditTab 用 `egui::TextEdit` 可通过注入 egui 事件实现；MergeTab 无选区模型，需先补）与 `移动标签页到新窗口 / 合并所有窗口`（2 项，需多窗口架构，当前 GUI 未使用 egui viewport）。
- **菜单展开态采集工具**：本轮临时工具在 `/tmp/bcrcmp/`（`bcrmenu`：AX dump / 真实鼠标点击展开 / ESC；`capture_menu.sh`：点击→全屏截图→像素校验→通过才落盘）。
...
- **文件夹合并视图过滤**：1-7 快捷键 + View 菜单（全部/更改/冲突/左变/右变/可合并/未变化，MergeFilter 枚举 + 渲染可见索引过滤）
- **文件夹比较过滤扩展**：独有/不独有/差异但无独有/左较新+左独有/右较新+右独有（ViewFilter 5 新变体 + View 菜单 10 项）
- **图片比较补齐**：重置差异偏移（滚动归零+定位第一差异）、比较元数据弹窗（格式/尺寸/大小/帧数 左右对比，差异红色）
- **表格/HEX/补丁/文本编辑补齐**：CsvTab 在后面插入列；HEX 复制到右边 ⇧⌃→（当前差异行左侧字节写右侧文件 + .bak）；PatchTab 选择选择内容（第一差异块选为选区 + 蓝色高亮）；TextEdit 使用选择内容查找 ⌘E（选区文本填入查找框）
